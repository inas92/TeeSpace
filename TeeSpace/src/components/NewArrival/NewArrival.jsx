...
const NewArrival = () => {
  const imagesData = [
    ...
    { src: arrival4, name: "Fine Jersey Tee",    oldPrice: '$30.00',
    newPrice: '$28.00', discountLabel:"-10%", isHot:true},
  ];
  return (
    <div className="arrival-wrapper">
      ...
      {imagesData.map((image , index) => (
         <div key={index} className={`image-card ${image.isHot ? 'hot-image' : ''}`}>
         {image.discountLabel && <div className="discount-label">{image.discountLabel}</div>}
         
         {image.isHot && (
            <div className={`hot-label ${image.discountLabel ? 'hot-label-with-discount' : ''}`}>
              Hot
            </div>
          )}
          ...
          <div className="image-details">
            ...
            <p className={`arrivalimage-price ${image.oldPrice ? 'newPrice' : ''}`}>
              {image.newPrice ? image.newPrice : image.price}
            </p>
            {image.oldPrice && (
              <p className="oldPrice">{image.oldPrice}</p>
            )}
            </div>
          </div>
        </div>
